...
{foreach name=langs from=$htmlitems.items key=lang item=langItems}
	...
	{foreach name=hooks from=$langItems key=hook item=hookItems}
		<h4 class="hook-title">{l s='Hook' mod='dev4webbanners'} "{$hook|escape:'htmlall':'UTF-8'}"</h4>
		{if $hookItems}
						...
						<form method="post" action="{$htmlitems.postAction|escape:'htmlall':'UTF-8'}" enctype="multipart/form-data" class="item-form defaultForm  form-horizontal">
							<div class="btn-group pull-right">
								<button class="btn btn-default button-edit">
									<span class="button-edit-edit"><i class="icon-edit"></i> {l s='Edit' mod='dev4webbanners'}</span>
									<span class="button-edit-close hide"><i class="icon-remove"></i> {l s='Close' mod='dev4webbanners'}</span>
								</button>
								...
								<ul class="dropdown-menu">
									<li>
										<a href="{$htmlitems.postAction|escape:'htmlall':'UTF-8'}&amp;removeItem&amp;item_id={$hItem.id_item|escape:'htmlall':'UTF-8'}" name="removeItem" class="link-item-delete">
											<i class="icon-trash"></i> {l s='Delete item' mod='dev4webbanners'}
										</a>
									</li>
								</ul>
							</div>
							...
							<div class="item-container clearfix">
								...
								<div class="hook item-field form-group">
									<label class="control-label col-lg-3">{l s='Hook to which the image should be attached' mod='dev4webbanners'}</label>
									<div class="col-lg-7">
										<select name="item_hook" default="top" class="fixed-width-lg">
											...
											<option value="product"{if $hItem.hook == 'product'} selected="selected"{/if}>product</option>
											<option value="product_footer"{if $hItem.hook == 'product_footer'} selected="selected"{/if}>product_footer</option>
										</select>
									</div>
								</div>
								<div class="image item-field form-group">
									<label class="control-label col-lg-3">{l s='Load your image' mod='dev4webbanners'}</label>
									<div class="col-lg-7">
								...
								<div class="image_w item-field form-group">
									...
									<div class="col-lg-7">
										<div class="input-group fixed-width-lg">
											...
											<span class="input-group-addon">{l s='pixels' mod='dev4webbanners'}</span>
										</div>
									</div>
								</div>
								...
								<div class="url item-field form-group">
									<label class="control-label col-lg-3">{l s='Target link' mod='dev4webbanners'}</label>
									<div class="col-lg-7">
								...
								<div class="target item-field form-group">
									<div class="col-lg-9 col-lg-offset-3">
										<div class="checkbox">
											<label class="control-label">
												{l s='Open link in a new tab/page' mod='dev4webbanners'}
								...
								<div class="html item-field form-group">
									<label class="control-label col-lg-3">{l s='Optional HTML code' mod='dev4webbanners'}</label>
									<div class="col-lg-7">
								...
								<div class="form-group">
									<div class="col-lg-7 col-lg-offset-3">
										<button type="button" class="btn btn-default button-item-edit-cancel" >
											<i class="icon-remove"></i> {l s='Cancel' mod='dev4webbanners'}
										</button>
										<button type="submit" name="updateItem" class="btn btn-success button-save pull-right" >
											<i class="icon-save"></i> {l s='Save' mod='dev4webbanners'}
										</button>
									</div>
								</div>
							</div>
		...
		{else}
			<div class="item">
				<span class="text-muted">{l s='No items available' mod='dev4webbanners'}</span>
			</div>
		{/if}
	{/foreach}
	</div>
